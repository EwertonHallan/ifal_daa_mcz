...
require_once('../../classe/coordenacao.php');
	...
	$msg .='			<th width="13%"><font size=2>E-MAIL</font></th>';
	...
	if (count($resultado)) {
	    ...
	    foreach ($resultado as $res) {
	        ...
	        $msg .=' <td align="right"><font size=2>'.$res['email'].' </font></td>';
	        $msg .=' <td align="center"><input type=button id=bt_sel'.$res['id_coordenacao'].' name=bt_sel'.$res['id_coordenacao'].' value="Selecionar" onclick="selCampo(paramRel_mapafaltas.id_coordenacao,paramRel_mapafaltas.desc_id_coordenacao,\''.$res['id_coordenacao'].'\',\''.$res['nome'].'\');" /></td>';
	        $msg .='</tr>';
