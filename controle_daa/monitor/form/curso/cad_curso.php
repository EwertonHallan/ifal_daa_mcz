...
if (!isset($_POST) || empty($_POST)) {
    ...
    if (!isset($dados['id_curso']) || !is_numeric($dados['id_curso'])) {
        $novo_cadastro = TRUE;
    }    
}
   ...
   if ($novo_cadastro) {
       ...
       if (!isset($result)) {
          ...
          $mensagem .= 'Inconsistencia no banco de dados!';
           ...
           header('Location: '.$_SESSION["dir_base_html"].'?form='.FuncaoCaracter::lowerTexto(get_class($grava)).'&modulo='.$_SESSION["dir_modulo"].'&pagina='.$dados['pagina'].'&editar_id='.$dados['id_curso']);
       }
   }     
}
